...
> 旧 `docs/design-spec.md`。**リニューアル前の1ページLP**（旧 `index.html`、ワイヤーフレーム案C「テック・グリッド」基準）のデザイン仕様の記録である。役割は **`DESIGN.md`** へ移行済みであり、**見た目を参照・変更するときは必ず `DESIGN.md` を見ること。** 本ファイルは過去の判断を遡るための記録として残している。
> （移動理由: 2026-08-15 のリニューアル公開により、旧LPは本番から姿を消し「現行サイトの記録」ではなくなった。移行期にあった「現行サイトの保守はこちらを正とする」という切り分けは解消済みで、見た目のSoTは `DESIGN.md` に一本化した。Phase 7 で `docs/archive/design-spec-lp-v1.md` へ移動・改名。継承／非継承の判断は `DESIGN.md` §12 にある。）
